  ...
  if List.length points >= 2 then
    ...
    List.for_all
      (fun p ->
        ...
        not (Float.is_nan result) && not (Float.is_infinite result)
...
let test_lagrange_interpolate_two_points p1 p2 x =
  if p1.x <> p2.x && x >= min p1.x p2.x && x <= max p1.x p2.x then (
    ...
    let eps = 0.0001 in
    not (Float.is_nan lagrange_result) && not (Float.is_infinite lagrange_result)
...
let test_newton_interpolate_two_points p1 p2 x =
  if p1.x <> p2.x && x >= min p1.x p2.x && x <= max p1.x p2.x then (
    ...
    let linear_result = linear_interpolate p1 p2 x in
    let eps = 0.0001 in
    not (Float.is_nan newton_result) && not (Float.is_infinite newton_result)
  ...
  [
    ...
    ( "lagrange_interpolate_two_points",
      ...
      fun () ->
        ...
        QCheck.Test.check_exn
          (QCheck.Test.make ~count:1000
             ~name:"lagrange_interpolate matches linear_interpolate for two points" gen
             (fun (p1, p2, x) -> test_lagrange_interpolate_two_points p1 p2 x)) );
    ...
    ( "newton_interpolate_two_points",
      ...
      fun () ->
        ...
        QCheck.Test.check_exn
          (QCheck.Test.make ~count:1000
             ~name:"newton_interpolate matches linear_interpolate for two points" gen
             (fun (p1, p2, x) -> test_newton_interpolate_two_points p1 p2 x)) );
    ( "lagrange_newton_equivalence",
      ...
      fun () ->
        ...
        QCheck.Test.check_exn
          (QCheck.Test.make ~count:500
             ~name:"lagrange_interpolate and newton_interpolate give same results"
