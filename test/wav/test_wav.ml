...
let test_read_wav () =
  ...
  check int "Number of channels is 2" info.Parser.num_channels 2;
  check int "Bits per sample is 16" info.Parser.bits_per_sample 16;
  check bool "Samples list is not empty"
    (List.length wav_data.Parser.samples > 0)
    true;
  ...
  check int "Correct number of samples"
    (List.length wav_data.Parser.samples)
    expected_samples

let test_write_read_roundtrip_16bit () =
  let test_samples = [ 0.0; 0.5; -0.5; 1.0; -1.0; 0.25; -0.25; 0.75 ] in
  let info =
    { Writer.sample_rate = 44100; num_channels = 1; bits_per_sample = 16 }
  in
  let test_file = Filename.temp_file "test_wav" ".wav" in
  try
    Writer.write_wav test_file info test_samples;
    let wav_data = Parser.read_wav test_file in
    check int "Roundtrip preserves sample rate"
      wav_data.Parser.info.Parser.sample_rate 44100;
    check int "Roundtrip preserves channels"
      wav_data.Parser.info.Parser.num_channels 1;
    check int "Roundtrip preserves bits per sample"
      wav_data.Parser.info.Parser.bits_per_sample 16;
    check int "Roundtrip preserves number of samples"
      (List.length wav_data.Parser.samples)
...
let test_write_read_roundtrip_32bit () =
  let test_samples = [ 0.0; 0.5; -0.5; 1.0; -1.0; 0.25; -0.25; 0.75 ] in
  let info =
    { Writer.sample_rate = 22050; num_channels = 1; bits_per_sample = 32 }
  in
  let test_file = Filename.temp_file "test_wav" ".wav" in
  try
    Writer.write_wav test_file info test_samples;
    let wav_data = Parser.read_wav test_file in
    check int "Roundtrip preserves sample rate"
      wav_data.Parser.info.Parser.sample_rate 22050;
    check int "Roundtrip preserves channels"
      wav_data.Parser.info.Parser.num_channels 1;
    check int "Roundtrip preserves bits per sample"
      wav_data.Parser.info.Parser.bits_per_sample 32;
    check int "Roundtrip preserves number of samples"
      (List.length wav_data.Parser.samples)
      (List.length test_samples);
...
let test_write_read_roundtrip_stereo () =
  ...
  let info =
    { Writer.sample_rate = 48000; num_channels = 2; bits_per_sample = 16 }
  in
  let test_file = Filename.temp_file "test_wav" ".wav" in
  try
    Writer.write_wav test_file info test_samples;
    let wav_data = Parser.read_wav test_file in
    check int "Roundtrip preserves sample rate"
      wav_data.Parser.info.Parser.sample_rate 48000;
    check int "Roundtrip preserves channels"
      wav_data.Parser.info.Parser.num_channels 2;
    check int "Roundtrip preserves bits per sample"
      wav_data.Parser.info.Parser.bits_per_sample 16;
    check int "Roundtrip preserves number of samples"
      (List.length wav_data.Parser.samples)
...
let test_write_read_roundtrip_32bit_precision () =
  ...
  let info =
    { Writer.sample_rate = 44100; num_channels = 1; bits_per_sample = 32 }
  in
  let test_file = Filename.temp_file "test_wav" ".wav" in
  try
    Writer.write_wav test_file info test_samples;
...
let test_clamp_samples () =
  ...
  try
    ...
    List.iter
      (fun sample ->
        check bool "Clamped sample in valid range"
...
let test_read_write_real_file () =
  ...
  try
    ...
    check int "Real file roundtrip preserves sample rate"
      recovered.Parser.info.Parser.sample_rate
      original.Parser.info.Parser.sample_rate;
    check int "Real file roundtrip preserves channels"
      recovered.Parser.info.Parser.num_channels
      original.Parser.info.Parser.num_channels;
    check int "Real file roundtrip preserves bits per sample"
      recovered.Parser.info.Parser.bits_per_sample
      original.Parser.info.Parser.bits_per_sample;
    check int "Real file roundtrip preserves number of samples"
      (List.length recovered.Parser.samples)
      (List.length original.Parser.samples);
    (* Compare samples (with quantization tolerance) *)
    List.iter2
      (fun original_sample recovered_sample ->
        check float_approx "Real file roundtrip preserves sample"
          original_sample recovered_sample)
...
let () =
  run "WAV Parser and Writer"
    [
      ( "WAV Parser",
        [
          test_case "Read WAV info" `Quick test_read_wav_info;
          test_case "Read WAV file" `Quick test_read_wav;
        ] );
      ( "WAV Writer - Roundtrip 16-bit",
        [
          test_case "Write/read roundtrip 16-bit mono" `Quick
            test_write_read_roundtrip_16bit;
          test_case "Write/read roundtrip 16-bit stereo" `Quick
            test_write_read_roundtrip_stereo;
        ] );
      ( "WAV Writer - Roundtrip 32-bit",
        [
          test_case "Write/read roundtrip 32-bit PCM" `Quick
            test_write_read_roundtrip_32bit;
          test_case "Write/read roundtrip 32-bit precision" `Quick
            test_write_read_roundtrip_32bit_precision;
        ] );
