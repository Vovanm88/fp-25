...
let test_dft_basic () =
  ...
  check int "DFT returns list of double length" (List.length result)
    (2 * List.length input);
  (* Check that result contains both real and imaginary parts *)
  check bool "DFT returns non-empty result" (List.length result > 0) true

(* Test DFT: fast vs reference *)
let test_dft_comparison () =
  let sizes = [ 4; 8 ] in
  (* Small sizes for basic correctness *)
  List.iter
    (fun n ->
      let data =
        List.init n (fun i ->
            Float.sin (2.0 *. Float.pi *. float_of_int i /. float_of_int n))
      in
      let fast_result = Dsp.dft_transform data in
      let reference_result = Dsp.dft_transform_reference data in
      check int
        (Printf.sprintf "DFT fast length matches reference for n=%d" n)
        (List.length fast_result)
        (List.length reference_result);
      ...
      in
      check bool
        (Printf.sprintf "DFT fast produces output for n=%d" n)
        (fast_sum > 0.0) true;
      check bool
        (Printf.sprintf "DFT ref produces output for n=%d" n)
        (ref_sum > 0.0) true)
    sizes
...
let test_idft_comparison () =
  ...
  List.iter
    (fun n ->
      ...
      check int
        (Printf.sprintf "IDFT fast length matches reference for n=%d" n)
        (List.length fast_result)
        (List.length reference_result);
      (* Check that both produce non-trivial output *)
      let fast_sum =
      ...
      in
      check bool
        (Printf.sprintf "IDFT fast produces output for n=%d" n)
        (fast_sum > 0.0) true;
      check bool
        (Printf.sprintf "IDFT ref produces output for n=%d" n)
        (ref_sum > 0.0) true)
    sizes
...
let test_idft_basic () =
  ...
  check int "IDFT returns list of half length" (List.length result)
    (List.length input / 2)
...
let test_dft_roundtrip () =
  ...
  check int "DFT-IDFT roundtrip preserves length" (List.length idft_result)
    (List.length input);
  (* Check that IDFT(DFT(x)) ≈ x *)
  List.iter2
    (fun original recovered ->
      check float_approx "DFT-IDFT roundtrip recovers original value" original
        recovered)
    input idft_result
...
let test_dft_roundtrip_sinusoid () =
  ...
  List.iter2
    (fun original recovered ->
      check float_approx "DFT-IDFT roundtrip on sinusoid" original recovered)
    input idft_result
...
let test_dft_roundtrip_constant () =
  ...
  List.iter2
    (fun original recovered ->
      check float_approx "DFT-IDFT roundtrip on constant signal" original
        recovered)
    input idft_result
...
let test_dft_roundtrip_complex_signal () =
  ...
  List.iter2
    (fun original recovered ->
      check float_relaxed "DFT-IDFT roundtrip on complex signal" original
        recovered)
    input idft_result
...
let test_dft_decompose_sinusoid () =
  ...
  let input =
    List.init n (fun k ->
        Float.sin
          (2.0 *. Float.pi *. float_of_int freq *. float_of_int k
         /. float_of_int n))
  in
  let dft_result = Dsp.dft_transform input in
  ...
  let magnitude_at_freq =
    Float.sqrt ((re_at_freq *. re_at_freq) +. (im_at_freq *. im_at_freq))
  in
  check float_approx "DFT detects frequency component"
    (if magnitude_at_freq > 1.0 then 1.0 else 0.0)
    1.0
...
let test_mdct_basic () =
  ...
  check int "MDCT returns half the length" (List.length result) expected_len

let test_imdct_basic () =
  let input = [ 1.0; 2.0 ] in
  let result = Dsp.imdct_transform input in
  let expected_len = 2 * List.length input in
  check int "IMDCT returns double the length" (List.length result) expected_len
...
let test_mdct_roundtrip () =
  ...
  check int "MDCT-IMDCT roundtrip preserves length" (List.length imdct_result)
    (List.length input);
  ...
  let imdct_has_nonzero =
    List.exists (fun x -> abs_float x > 0.001) imdct_result
  in
...
let test_mdct_single_element () =
  ...
  let input = [ 1.0; 2.0 ] in
  let mdct_result = Dsp.mdct_transform input in
  check int "MDCT on 2 elements returns 1 element" (List.length mdct_result) 1;
  let imdct_result = Dsp.imdct_transform mdct_result in
  check int "IMDCT on 1 element returns 2 elements" (List.length imdct_result) 2
...
let test_mdct_sinusoid () =
  ...
  in
  let mdct_result = Dsp.mdct_transform input in
  check int "MDCT on sinusoid returns correct length" (List.length mdct_result)
    (n / 2);
  (* MDCT coefficients should be non-zero *)
  let has_nonzero = List.exists (fun x -> abs_float x > 0.001) mdct_result in
...
let test_fft_shift () =
  ...
  check int "FFT shift preserves length" (List.length result)
    (List.length input);
  check (list float_approx) "FFT shift moves elements" result
    [ 3.0; 4.0; 1.0; 2.0 ]

let test_hamming_window () =
  let input = [ 1.0; 1.0; 1.0; 1.0 ] in
  let result = Dsp.hamming_window input in
  check int "Hamming window preserves length" (List.length result)
    (List.length input);
  (* Hamming window should modify values *)
  check bool "Hamming window modifies values" (result <> input) true

let test_hanning_window () =
  let input = [ 1.0; 1.0; 1.0; 1.0 ] in
  let result = Dsp.hanning_window input in
  check int "Hanning window preserves length" (List.length result)
    (List.length input);
  (* Hanning window should modify values *)
  check bool "Hanning window modifies values" (result <> input) true

let test_rectangular_window () =
  let input = [ 1.0; 2.0; 3.0; 4.0 ] in
  let result = Dsp.rectangular_window input in
  check (list float_approx) "Rectangular window returns same list" result input
...
let test_single_element () =
  ...
  check (list float_approx) "DFT on single element" dft_result [ 5.0; 0.0 ];
  let idft_result = Dsp.idft_transform dft_result in
  check (list float_approx) "IDFT on single element" idft_result [ 5.0 ];
  check (list float_approx) "Hamming window on single element"
    (Dsp.hamming_window single)
    [ 5.0 ]
...
let test_filter_bank_basic () =
  ...
  check int "Filter bank returns correct number of bands" (List.length result)
    num_bands;
  (* Each band should have the same length as input *)
  List.iter
    (fun band ->
      check int "Filter bank band preserves length" (List.length band)
        (List.length input))
    result
...
let test_filter_bank_single_band () =
  ...
  check int "Filter bank with 1 band returns 1 band" (List.length result) 1;
...
let test_filter_bank_multiple_bands () =
  ...
  let num_bands = 4 in
  let result = Dsp.filter_bank input num_bands in
  check int "Filter bank with 4 bands returns 4 bands" (List.length result)
    num_bands;
  (* Each band should have the same length *)
  List.iter
    (fun band ->
      check int "Filter bank band has correct length" (List.length band)
        (List.length input))
    result;
  (* Bands should sum approximately to original (with some error due to filtering) *)
  let summed =
    List.fold_left
      (fun acc band -> List.map2 ( +. ) acc band)
  ...
  let max_diff =
    List.fold_left2
      (fun acc orig recon -> max acc (abs_float (orig -. recon)))
      0.0 input summed
  in
...
let test_filter_bank_empty () =
  ...
  check (list (list float_approx)) "Filter bank on empty list" result []

let test_filter_bank_zero_bands () =
  let input = [ 1.0; 2.0; 3.0 ] in
  let result = Dsp.filter_bank input 0 in
  check (list (list float_approx)) "Filter bank with 0 bands" result []
...
let test_filter_bank_custom_basic () =
  ...
  List.iter
    (fun band ->
      check int "Filter bank custom band preserves length" (List.length band)
        (List.length input))
    result
...
let test_filter_bank_custom_empty () =
  ...
  check (list (list float_approx)) "Filter bank custom on empty list" result []

let test_filter_bank_custom_no_bands () =
  let input = [ 1.0; 2.0; 3.0 ] in
  let result = Dsp.filter_bank_custom input [] in
  check (list (list float_approx)) "Filter bank custom with no bands" result []

let test_filter_bank_custom_overlapping () =
  let input = List.init 16 (fun i -> float_of_int i) in
  let bands = [ (0.0, 0.6); (0.4, 1.0) ] in
  let result = Dsp.filter_bank_custom input bands in
  check int "Filter bank custom with overlapping bands" (List.length result)
    (List.length bands);
...
let test_snr_identical () =
  ...
  check bool "SNR is infinite for identical signals"
    (Float.is_infinite snr_val)
    true
...
let test_mdct_quantization_snr () =
  ...
  let snr_values =
    List.map
      (fun num_levels ->
        ...
        let quantized, min_val, max_val =
          Quantization.quantize_with_range mdct_coeffs num_levels
        in
        (* Dequantize *)
        let dequantized =
          Quantization.dequantize_with_range quantized num_levels min_val
            max_val
        in
        (* Apply IMDCT *)
  ...
  check bool "Low quantization gives finite SNR" (Float.is_finite low_snr) true
...
let test_mdct_quantization_levels () =
  ...
  check bool "128 levels generally better than 8 levels"
    (snr_128 >= snr_8 -. 10.0)
    true
(* Allow tolerance for MDCT artifacts *)
...
let test_mdct_quantization_roundtrip () =
  ...
  in
  let dequantized =
    Quantization.dequantize_with_range quantized num_levels min_val max_val
  in
  let reconstructed = Dsp.imdct_transform dequantized in
  ...
  check bool "SNR is finite and positive for 64 levels"
    (Float.is_finite snr_val && snr_val > 0.0)
    true
...
let test_haar_basic () =
  ...
  check int "Haar transform returns approximations"
    (List.length approximations)
    1;
  check int "Haar transform returns details" (List.length details) 3;
  (* For [1, 2, 3, 4]:
...
let test_haar_inverse_basic () =
  ...
  check int "Haar inverse preserves length" (List.length reconstructed) 4;
  (* Should reconstruct [1.0, 2.0, 3.0, 4.0] *)
  check (list float_approx) "Haar inverse reconstructs correctly" reconstructed
    [ 1.0; 2.0; 3.0; 4.0 ]
...
let test_haar_roundtrip () =
  ...
  List.iter2
    (fun orig recov ->
      check float_approx "Haar roundtrip recovers original" orig recov)
    input reconstructed

let test_haar_empty () =
  let empty = [] in
  let appr, detail = Dsp.haar_transform empty in
  check (list float_approx) "Haar transform on empty list" appr [];
  check (list float_approx) "Haar transform details on empty list" detail [];
  let reconstructed = Dsp.haar_inverse_transform [] [] in
  check (list float_approx) "Haar inverse on empty lists" reconstructed []

let test_haar_single () =
  let single = [ 5.0 ] in
  let appr, detail = Dsp.haar_transform single in
  check (list float_approx) "Haar transform on single element" appr [ 5.0 ];
  check (list float_approx) "Haar transform details on single element" detail [];
  let reconstructed = Dsp.haar_inverse_transform [ 5.0 ] [] in
  check (list float_approx) "Haar inverse on single element" reconstructed
    [ 5.0 ]
...
let test_haar_power_of_2 () =
  ...
  List.iter2
    (fun orig recov ->
      check float_approx "Haar roundtrip on power of 2" orig recov)
    input reconstructed

let test_dwt_haar_basic () =
  let input = [ 1.0; 2.0; 3.0; 4.0 ] in
  let dwt_result = Dsp.dwt_haar input in
  check int "DWT Haar returns same length" (List.length dwt_result)
    (List.length input);
  (* DWT should be [2.5, -1.0, -0.5, -0.5] *)
  check (list float_approx) "DWT Haar correct values" dwt_result
    [ 2.5; -1.0; -0.5; -0.5 ]
...
let test_idwt_haar_basic () =
  ...
  check int "IDWT Haar returns correct length" (List.length reconstructed) 4;
  check (list float_approx) "IDWT Haar reconstructs correctly" reconstructed
    [ 1.0; 2.0; 3.0; 4.0 ]
...
let test_dwt_idwt_haar_roundtrip () =
  ...
  check int "DWT-IDWT Haar roundtrip preserves length" (List.length idwt_result)
    (List.length input);
  List.iter2
    (fun orig recov -> check float_approx "DWT-IDWT Haar roundtrip" orig recov)
    input idwt_result

let test_dwt_haar_empty () =
  let empty = [] in
  let dwt_result = Dsp.dwt_haar empty in
  check (list float_approx) "DWT Haar on empty list" dwt_result [];
  let idwt_result = Dsp.idwt_haar [] in
  check (list float_approx) "IDWT Haar on empty list" idwt_result []

let test_dwt_haar_single () =
  let single = [ 5.0 ] in
  let dwt_result = Dsp.dwt_haar single in
  check (list float_approx) "DWT Haar on single element" dwt_result [ 5.0 ];
  let idwt_result = Dsp.idwt_haar [ 5.0 ] in
  check (list float_approx) "IDWT Haar on single element" idwt_result [ 5.0 ]
...
let () =
  run "DSP Tests"
    [
      ...
      ( "DFT roundtrip tests",
        [
          test_case "DFT-IDFT roundtrip on sinusoid" `Quick
            test_dft_roundtrip_sinusoid;
          test_case "DFT-IDFT roundtrip on constant" `Quick
            test_dft_roundtrip_constant;
          test_case "DFT-IDFT roundtrip on complex signal" `Quick
            test_dft_roundtrip_complex_signal;
        ] );
      ...
      ("FFT shift", [ test_case "FFT shift" `Quick test_fft_shift ]);
      ( "Window functions",
        [
      ...
      ( "Filter bank",
        [
          ...
          test_case "Filter bank single band" `Quick
            test_filter_bank_single_band;
          test_case "Filter bank multiple bands" `Quick
            test_filter_bank_multiple_bands;
      ...
      ( "Filter bank custom",
        [
          test_case "Filter bank custom basic" `Quick
            test_filter_bank_custom_basic;
          test_case "Filter bank custom empty" `Quick
            test_filter_bank_custom_empty;
          test_case "Filter bank custom no bands" `Quick
            test_filter_bank_custom_no_bands;
          test_case "Filter bank custom overlapping" `Quick
            test_filter_bank_custom_overlapping;
          test_case "Filter bank custom full range" `Quick
            test_filter_bank_custom_full_range;
        ] );
      ( "SNR",
        [
      ...
      ( "E2E MDCT + Quantization",
        [
          test_case "MDCT quantization SNR degradation" `Quick
            test_mdct_quantization_snr;
          test_case "MDCT quantization levels comparison" `Quick
            test_mdct_quantization_levels;
          test_case "MDCT quantization roundtrip" `Quick
            test_mdct_quantization_roundtrip;
        ] );
      ( "Haar Wavelet Transform",
        [
          test_case "Haar transform basic" `Quick test_haar_basic;
          test_case "Haar inverse transform basic" `Quick
            test_haar_inverse_basic;
      ...
      ( "DWT Haar",
        [
          ...
          test_case "DWT-IDWT Haar roundtrip" `Quick
            test_dwt_idwt_haar_roundtrip;
