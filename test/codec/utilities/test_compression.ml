...
let test_bytes_to_bools_basic () =
  ...
  check int "Bytes to bools returns 8 bools per byte" (List.length bools)
    (8 * List.length bytes)
...
let test_bools_bytes_roundtrip () =
  ...
  let bools2_trunc =
    if List.length bools2 >= original_len then
...
let test_huffman_compression_e2e () =
  ...
  let compression_ratio =
    float_of_int encoded_size_bits /. float_of_int input_size_bits
  in
  ...
  check bool "Huffman encoding compresses or maintains size"
    (encoded_size_bits <= input_size_bits)
    true;
  (* Verify roundtrip *)
  let decoded = Compression.decode encoded tree in
...
let test_huffman_compression_text () =
  ...
  check bool "Huffman achieves good compression ratio" (compression_ratio < 0.8)
    true (* At least 20% compression *)

let test_huffman_compression_repeated () =
  (* Very redundant data - should compress well *)
  let input =
    List.init 100 (fun _ -> 'a')
    @ List.init 50 (fun _ -> 'b')
    @ List.init 25 (fun _ -> 'c')
  in
  let input_size_bits = List.length input * 8 in
...
let test_encode_bytes_single_byte () =
  ...
  check (list int) "Encode_bytes-decode_bytes single byte roundtrip" decoded
    input

let test_encode_bytes_roundtrip () =
  let input = [ 65; 66; 67; 65; 66 ] in
  (* 'A', 'B', 'C', 'A', 'B' *)
  let encoded, num_bits, tree = Compression.encode_bytes input in
  let decoded = Compression.decode_bytes encoded num_bits tree in
...
let test_encode_bytes_all_values () =
  ...
  let input = [ 65; 66; 67; 68 ] in
  (* 'A', 'B', 'C', 'D' - all different *)
  let encoded, num_bits, tree = Compression.encode_bytes input in
  let decoded = Compression.decode_bytes encoded num_bits tree in
  check (list int) "Encode_bytes-decode_bytes all values roundtrip" decoded
    input

let test_encode_bytes_repeated () =
  let input = [ 65; 65; 66; 66; 66; 67 ] in
  (* 'A', 'A', 'B', 'B', 'B', 'C' *)
  let encoded, num_bits, tree = Compression.encode_bytes input in
  let decoded = Compression.decode_bytes encoded num_bits tree in
...
let test_encode_bytes_invalid_negative () =
  ...
  with
  | Failure msg ->
      check bool "Fails on negative byte" (String.contains msg 'n') true
...
let test_serialize_tree_leaf () =
  ...
  check bool "Serialize_tree leaf returns non-empty"
    (List.length serialized > 0)
    true;
...
let test_serialize_tree_node () =
  ...
  let serialized = Compression.serialize_tree tree in
  check bool "Serialize_tree node returns non-empty"
    (List.length serialized > 0)
    true;
...
let test_serialize_tree_complex () =
  ...
  let serialized = Compression.serialize_tree tree in
  check bool "Serialize_tree complex returns non-empty"
    (List.length serialized > 0)
    true
...
let test_deserialize_tree_leaf () =
  ...
  | _ ->
      check bool "Deserialize_tree leaf type" false true;
...
let test_deserialize_tree_node () =
  ...
  | _ ->
      check bool "Deserialize_tree node type" false true;
...
let test_serialize_deserialize_roundtrip () =
  ...
  let rec trees_equal t1 t2 =
    match (t1, t2) with
    | Compression.Leaf (c1, f1), Compression.Leaf (c2, f2) -> c1 = c2 && f1 = f2
    | Compression.Node (f1, l1, r1), Compression.Node (f2, l2, r2) ->
        f1 = f2 && trees_equal l1 l2 && trees_equal r1 r2
    | _ -> false
  in
  check bool "Serialize-deserialize roundtrip"
    (trees_equal tree deserialized)
    true;
...
let test_compression_full_cycle () =
  ...
  | Some tree ->
      ...
      check bool "Full cycle: serialize_tree returns non-empty"
        (List.length serialized_tree > 0)
        true;
      (* Deserialize tree *)
      let deserialized_tree, remaining =
        Compression.deserialize_tree serialized_tree
      in
      ...
      let decoded_original =
        Compression.decode_bytes encoded_bytes num_bits (Some tree)
      in
      check (list int) "Full cycle: decode with original tree" decoded_original
        input;
      (* Then decode using deserialized tree *)
      let decoded =
        Compression.decode_bytes encoded_bytes num_bits (Some deserialized_tree)
      in
      check (list int)
        "Full cycle: encode->serialize->deserialize->decode roundtrip" decoded
        input
    ...
    [
      ( "Basic encoding/decoding",
        [
          ...
          test_case "Encode-decode single char" `Quick
      ...
      ( "Encode bytes",
        [
          ...
          test_case "Encode_bytes single byte" `Quick
            test_encode_bytes_single_byte;
          test_case "Encode_bytes roundtrip" `Quick test_encode_bytes_roundtrip;
          test_case "Encode_bytes all values" `Quick
            test_encode_bytes_all_values;
          test_case "Encode_bytes repeated" `Quick test_encode_bytes_repeated;
          test_case "Encode_bytes invalid negative" `Quick
            test_encode_bytes_invalid_negative;
          test_case "Encode_bytes invalid too large" `Quick
            test_encode_bytes_invalid_too_large;
        ] );
      ( "Serialize tree",
        [
      ...
      ( "Deserialize tree",
        [
          ...
          test_case "Serialize-deserialize roundtrip" `Quick
      ...
      ( "E2E Compression",
        [
          test_case "Huffman compression E2E" `Quick
            test_huffman_compression_e2e;
          test_case "Huffman compression text" `Quick
            test_huffman_compression_text;
          test_case "Huffman compression repeated" `Quick
            test_huffman_compression_repeated;
          test_case "Huffman no compression edge case" `Quick
            test_huffman_no_compression_edge_case;
        ] );
    ]
