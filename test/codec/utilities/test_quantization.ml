...
let test_quantize_basic () =
  ...
  check int "Quantize returns correct length" (List.length quantized)
    (List.length input);
...
let test_dequantize_basic () =
  ...
  let max_val = 4.0 in
  let dequantized =
    Quantization.dequantize quantized num_levels min_val max_val
  in
  check int "Dequantize returns correct length" (List.length dequantized)
...
let test_quantize_dequantize_roundtrip () =
  ...
  List.iter2
    (fun orig dq -> check quant_error "Quantize-dequantize roundtrip" orig dq)
    input dequantized
...
let test_quantize_with_range () =
  ...
  let quantized, min_val, max_val =
    Quantization.quantize_with_range input num_levels
  in
  check int "Quantize with range returns correct length" (List.length quantized)
    (List.length input);
...
let test_quantize_custom_range () =
  ...
  check int "Quantize clamps values" (List.length quantized2)
    (List.length input2)
...
let test_quantize_constant_signal () =
  ...
  let num_levels = 4 in
  let quantized = Quantization.quantize input num_levels in
  check int "Quantize constant signal" (List.length quantized)
    (List.length input);
...
let test_quantize_high_levels () =
  ...
  let quantized = Quantization.quantize input num_levels in
  check int "Quantize with many levels" (List.length quantized)
    (List.length input);
...
let test_encode_decode_roundtrip () =
  ...
  let decoded =
    Quantization.decode_quantized_indices encoded (List.length indices) n
  in
...
let test_encode_clamping () =
  ...
  let indices = [ -1; 0; 5; 3; 10 ] in
  (* n=4, so valid range is [0,3] *)
  let n = 4 in
  let encoded = Quantization.encode_quantized_indices indices n in
  let decoded =
    Quantization.decode_quantized_indices encoded (List.length indices) n
    ...
    [
      ( "Basic quantization",
        [
          ...
          test_case "Quantize-dequantize roundtrip" `Quick
      ...
      ( "Edge cases",
        [
          ...
          test_case "Quantize constant signal" `Quick
            test_quantize_constant_signal;
      ...
      ( "Encoding/Decoding",
        [
          test_case "Encode-decode roundtrip" `Quick
