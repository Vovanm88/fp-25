...
let test_quantization_levels_roundtrip () =
  ...
  check int "Levels count matches" (List.length read_levels)
    (List.length levels);

  (* Check each level *)
  List.iter2
    (fun orig read -> check int "Level matches" read orig)
    levels read_levels
...
let () =
  run "Simple Serialization Tests"
    [
      ( "Basic",
        [
          test_case "quantization_levels roundtrip" `Quick
            test_quantization_levels_roundtrip;
        ] );
    ]
