...
let () =
  Printf.printf
    "\n╔════════════════════════════════════════════════════════════╗\n";
  Printf.printf
    "║         WAV to AUDC Converter (3 Quality Modes)           ║\n";
  Printf.printf
    "╚════════════════════════════════════════════════════════════╝\n\n";
  ...
  (* Process each mode *)
  List.iter
    (fun (mode_id, mode_name, snr_threshold, emoji) ->
      Printf.printf
        "════════════════════════════════════════════════════════════\n";
      ...
      in
      let decoded_file =
        Filename.concat output_dir (Printf.sprintf "test_sample_%s.wav" mode_id)
      in
      ...
      let _audio_file =
        Encoder.encode_to_file ~snr_threshold_db:snr_threshold mono_samples
      ...
      Printf.printf "   ✓ Size: %d bytes (%.2f KB)\n" encoded_size
        (float_of_int encoded_size /. 1024.0);
      ...
      in
      let min_len =
        min (List.length mono_samples) (List.length reconstructed)
      in
      ...
      Printf.printf "   ✓ File: %s\n" decoded_file;
      Printf.printf "   ✓ Size: %d bytes (%.2f KB)\n" decoded_size
        (float_of_int decoded_size /. 1024.0);

      (* Calculate compression ratio *)
      let original_size = (List.length mono_samples * 2) + 44 in
      ...
      Printf.printf "   Original size (approx): %d bytes (%.2f KB)\n"
        original_size
        (float_of_int original_size /. 1024.0);
