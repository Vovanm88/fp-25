...
let test_compression_high_quality () =
  ...
  try
    ...
    let audio_file =
      Encoder.encode_to_file ~snr_threshold_db:60.0 test_samples sample_rate
        temp_encoded
    in

    (* Check encoded file *)
    check bool "Encoded file exists" (Sys.file_exists temp_encoded) true;
    check int "Encoded file has 1 track" audio_file.Codec.Audiomodel.num_tracks
      1;
    ...
    check bool "Decoded signal is not empty"
...
let test_compression_medium_quality () =
  ...
  try
    ...
    let audio_file =
      Encoder.encode_to_file ~snr_threshold_db:40.0 test_samples sample_rate
        temp_encoded
    in
...
let test_compression_low_quality () =
  ...
  try
    ...
    let audio_file =
      Encoder.encode_to_file ~snr_threshold_db:20.0 test_samples sample_rate
        temp_encoded
    in
...
let test_compression_full_wav () =
  ...
  try
    ...
    let audio_file =
      Encoder.encode_to_file test_samples sample_rate temp_encoded
    in
    ...
    check int "Decoded WAV length matches"
      (List.length decoded_wav.Wav.Parser.samples)
      (List.length reconstructed);
...
let test_compression_comparison () =
  ...
  List.iter
    (fun (mode_name, snr_threshold) ->
      let temp_encoded = Filename.temp_file "e2e_comparison" ".audc" in
      try
        (* Encode *)
        let _audio_file =
          Encoder.encode_to_file ~snr_threshold_db:snr_threshold test_samples
            sample_rate temp_encoded
        in
        let encoded_size = (Unix.stat temp_encoded).Unix.st_size in
        ...
        (* Calculate SNR *)
        let min_len =
          min (List.length test_samples) (List.length reconstructed)
        in
        let input_trimmed =
          List.init min_len (fun i -> List.nth test_samples i)
        in
        ...
        let original_size = (List.length test_samples * 2) + 44 in
        (* 16-bit samples + WAV header *)
        let compression_ratio =
          float_of_int original_size /. float_of_int encoded_size
        in
...
let () =
  run "E2E WAV Compression Tests"
    [
      ( "Compression Modes",
        [
          test_case "High quality compression (60 dB)" `Slow
            test_compression_high_quality;
          test_case "Medium quality compression (40 dB)" `Slow
            test_compression_medium_quality;
          test_case "Low quality compression (20 dB)" `Slow
            test_compression_low_quality;
        ] );
      ( "Full WAV Tests",
        [
      ...
      ( "Comparison",
        [
          test_case "Compare compression modes" `Slow
