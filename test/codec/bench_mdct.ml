...
let bench_mdct_comparison () =
  ...
  List.iter
    (fun size ->
      ...
      let fast_time, fast_result =
        time_function (fun () -> Dsp.mdct_transform data)
      in

      (* Benchmark reference implementation *)
      let ref_time, ref_result =
        time_function (fun () -> Dsp.mdct_transform_reference data)
      in
      ...
      (* Verify correctness *)
      let max_diff =
        if List.length fast_result = List.length ref_result then
          List.fold_left2
  ...
  List.iter
    (fun size ->
      ...
      let fast_time, _fast_result =
        time_function (fun () -> Dsp.imdct_transform data)
      in

      (* Benchmark reference implementation *)
      let ref_time, _ref_result =
        time_function (fun () -> Dsp.imdct_transform_reference data)
      in
...
let bench_realistic_sizes () =
  ...
  let durations = [ 0.01; 0.05; 0.1 ] in
  (* seconds *)
  ...
  List.iter
    (fun duration ->
      ...
      let rec nearest_pow2 n acc =
        if acc >= n then acc else nearest_pow2 n (acc * 2)
      in
