...
let bench_dft_comparison () =
  ...
  List.iter (fun size ->
    ...
    let (fast_time, _fast_result) = time_function (fun () ->
      Dsp.dft_transform data
    ) in
    
    (* Benchmark reference implementation *)
    let (ref_time, _ref_result) = time_function (fun () ->
      Dsp.dft_transform_reference data
    ) in
...
let bench_idft_comparison () =
  ...
  List.iter (fun size ->
    ...
    (* Benchmark fast implementation *)
    let (fast_time, _fast_result) = time_function (fun () ->
      Dsp.idft_transform dft_data
    ) in
    
    (* Benchmark reference implementation *)
    let (ref_time, _ref_result) = time_function (fun () ->
      Dsp.idft_transform_reference dft_data
    ) in
...
let bench_filter_bank_sizes () =
  ...
  List.iter (fun (size, desc) ->
    ...
    Printf.printf "  %5d  | %8.6f | %8.6f | %7.2fx | %s\n"
      size fast_time ref_time speedup desc
  ) test_cases;
