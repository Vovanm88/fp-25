...
let ensure_output_dir () =
  ...
  (try if not (Sys.file_exists output_dir) then Unix.mkdir output_dir 0o755
   with _ -> ());
  output_dir
...
let test_golden_compression () =
  ...
  List.iter
    (fun (mode_id, mode_name, snr_threshold) ->
      ...
      (* Verify files exist *)
      check bool
        (Printf.sprintf "%s: encoded file exists" mode_name)
        (Sys.file_exists encoded_file)
        true;
      check bool
        (Printf.sprintf "%s: decoded file exists" mode_name)
        (Sys.file_exists decoded_file)
        true;
      check int
        (Printf.sprintf "%s: audio file has 1 track" mode_name)
        audio_file.Codec.Audiomodel.num_tracks 1;
      check int
        (Printf.sprintf "%s: decoded sample rate matches" mode_name)
  ...
  Printf.printf "  - High quality: %s/test_sample_high_quality_decoded.wav\n"
    output_dir;
  Printf.printf
    "  - Medium quality: %s/test_sample_medium_quality_decoded.wav\n" output_dir;
...
let test_golden_full_wav () =
  ...
  let decoded_file =
    Filename.concat output_dir "test_sample_full_decoded.wav"
  in

  (* Encode with default quality *)
  Printf.printf "Encoding full WAV file...\n";
  let audio_file =
    Encoder.encode_to_file mono_samples sample_rate encoded_file
  in
  let encoded_size = (Unix.stat encoded_file).Unix.st_size in
  ...
  check int "Full WAV: audio file has 1 track"
    audio_file.Codec.Audiomodel.num_tracks 1;
  check int "Full WAV: decoded sample rate matches" decoded_sample_rate
    sample_rate
...
let () =
  run "Golden WAV Compression Tests"
    [
      ( "Golden Tests",
        [
          test_case "Compress and decompress in different modes" `Slow
            test_golden_compression;
