...
let test_mdct_comparison () =
  ...
  List.iter (fun size ->
    ...
    check int (Printf.sprintf "MDCT output size matches for n=%d" size) ref_len fast_len;
    ...
    check bool (Printf.sprintf "MDCT fast produces output for n=%d" size) (fast_sum > 0.0) true;
    check bool (Printf.sprintf "MDCT ref produces output for n=%d" size) (ref_sum > 0.0) true
  ) sizes
...
let test_imdct_comparison () =
  ...
  List.iter (fun size ->
    ...
    let fast_len = List.length fast_result in
    let ref_len = List.length reference_result in
    check int (Printf.sprintf "IMDCT output size matches for m=%d" size) ref_len fast_len;
    (* Check that both produce non-trivial output *)
    let fast_sum = List.fold_left (fun acc x -> acc +. abs_float x) 0.0 fast_result in
    let ref_sum = List.fold_left (fun acc x -> acc +. abs_float x) 0.0 reference_result in
    check bool (Printf.sprintf "IMDCT fast produces output for m=%d" size) (fast_sum > 0.0) true;
    check bool (Printf.sprintf "IMDCT ref produces output for m=%d" size) (ref_sum > 0.0) true
  ) sizes
  ...
  List.iter (fun size ->
    ...
    if List.length mdct_result > 0 then (
      ...
      if min_len > 0 then (
        ...
        check bool (Printf.sprintf "Fast MDCT roundtrip for n=%d" size)
          (max_diff < 5.0) true
      )
    )
  ) sizes
...
let test_mdct_roundtrip_reference () =
  ...
  List.iter (fun size ->
    ...
    if min_len > 0 then (
      ...
      (* MDCT roundtrip without overlap-add won't be perfect *)
      check bool (Printf.sprintf "Reference MDCT roundtrip for n=%d" size)
        (max_diff < 5.0) true
    )
  ) sizes
