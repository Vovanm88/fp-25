...
let test_encoder_compress_basic () =
  ...
  check int "Compressed length matches input" (List.length compressed) (List.length input);
  ...
  check bool "Roundtrip SNR is very high or infinity" 
    (snr_db >= 100.0 || Float.is_infinite snr_db) true
...
let test_encoder_compress_roundtrip () =
  ...
  check bool "Roundtrip SNR is very high or infinity" 
    (roundtrip_snr >= 200.0 || Float.is_infinite roundtrip_snr) true;
  ...
  check bool "SNR from compress_track matches manual roundtrip" 
    (abs_float (snr_db -. roundtrip_snr) < 0.01 || 
     (Float.is_infinite snr_db && Float.is_infinite roundtrip_snr)) true
...
let test_encoder_compress_stereo () =
  ...
  let (compressed_l, compressed_r), (_min_l, _max_l), (_min_r, _max_r), snr_avg = 
    Encoder.compress_stereo_tracks left right in
  check int "Compressed left length" (List.length compressed_l) (List.length left);
  check int "Compressed right length" (List.length compressed_r) (List.length right);
  (* Average SNR should be very high or infinity for lossless compression *)
  check bool "Average SNR is very high or infinity" 
    (snr_avg >= 100.0 || Float.is_infinite snr_avg) true
...
let test_decoder_decompress_e2e () =
  ...
  (* Check roundtrip *)
  check int "Decompressed length matches input" (List.length decompressed) (List.length input);
  let roundtrip_snr = Dsp.snr input decompressed in
...
let test_encoder_lr_to_mid_side_stereo () =
  ...
  List.iter2 (fun exp act -> check (float 0.0001) "Mid value" act exp) expected_mid mid;
  List.iter2 (fun exp act -> check (float 0.0001) "Side value" act exp) expected_side side
...
let test_encoder_lr_to_mid_side_mono () =
  ...
  List.iter2 (fun exp act -> check (float 0.0001) "Mid equals input for mono" act exp) mono mid;
  List.iter2 (fun exp act -> check (float 0.0001) "Side equals input for mono" act exp) mono side
...
let test_decoder_stereo_e2e () =
  ...
  check bool "E2E roundtrip SNR left is very high" 
    (snr_left >= 200.0 || Float.is_infinite snr_left) true;
  check bool "E2E roundtrip SNR right is very high" 
    (snr_right >= 200.0 || Float.is_infinite snr_right) true
...
let test_decoder_mono_e2e () =
  ...
  let decoded_left, _decoded_right = Decoder.mid_side_to_lr mid side is_stereo in
  ...
  check bool "E2E roundtrip SNR is very high" 
    (snr >= 200.0 || Float.is_infinite snr) true
...
let test_encoder_silence_replacement () =
  ...
  let processed, silence_mask = Encoder.replace_silence_after_loud 
      ~loud_threshold:0.1 ~quiet_threshold:0.01 ~quiet_duration_ms:50.0 
      sample_rate input in
  check int "Processed length matches input" (List.length processed) (List.length input);
  check int "Silence mask length matches input" (List.length silence_mask) (List.length input);
  ...
  let all_silenced_zero = List.for_all2 (fun p m -> if m then abs_float p < 1e-10 else true) processed silence_mask in
...
let test_encoder_silence_no_replacement () =
  ...
  let processed, silence_mask = Encoder.replace_silence_after_loud sample_rate input in
  check int "Processed length matches input" (List.length processed) (List.length input);
  (* No silence should be replaced *)
  let silenced_count = List.fold_left (fun acc s -> if s then acc + 1 else acc) 0 silence_mask in
...
let test_decoder_silence_e2e () =
  ...
  let decoded_left, _decoded_right = Decoder.mid_side_to_lr passed side is_stereo in
  let final = Decoder.decompress_track decoded_left min_val max_val in
  ...
  check bool "E2E roundtrip SNR is acceptable" (snr >= 20.0 || Float.is_infinite snr) true

(* Step 3.1: Encoder segmentation (optimized - smaller input) *)
let test_encoder_segmentation_basic () =
  let sample_rate = 44100 in
  let input = List.init 1000 (fun i -> Float.sin (2.0 *. Float.pi *. float_of_int i /. 100.0)) in
  let segmented, segments = Encoder.segment_track sample_rate input in
  check int "Segmented length >= input length" (List.length segmented) (List.length input);
  ...
  check bool "Attack time is non-negative" (seg.Audiomodel.attack_ms >= 0.0) true;
  check bool "Release time is non-negative" (seg.Audiomodel.release_ms >= 0.0) true

let test_encoder_segmentation_short_track () =
  let sample_rate = 44100 in
  (* Short track that needs padding *)
  let input = List.init 100 (fun _ -> 0.5) in
  let segmented, segments = Encoder.segment_track ~min_segment_length:512 sample_rate input in
  check int "Short track is padded to min_segment_length" (List.length segmented) 512;
  check bool "Has segment info" (List.length segments > 0) true;
  let seg = List.hd segments in
  (* segments is segment_info list *)
  check bool "Original length is recorded" (seg.Audiomodel.original_length = Some 100) true

(* Step 3.2: Encoder overlapping windows (optimized - smaller input) *)
let test_encoder_overlapping_windows () =
  let sample_rate = 44100 in
  let input = List.init 1500 (fun i -> Float.sin (2.0 *. Float.pi *. float_of_int i /. 100.0)) in
  ...
  List.iter (fun win ->
    check bool "Window has window_data" 
      (match win.Audiomodel.window_data with Some _ -> true | None -> false) true;
    check bool "Window has start_sample" 
      (match win.Audiomodel.start_sample with Some _ -> true | None -> false) true
  ) windows;
  ...
  if List.length windows >= 2 then (
    ...
    let start1 = match win1.Audiomodel.start_sample with Some x -> x | None -> 0 in
    let start2 = match win2.Audiomodel.start_sample with Some x -> x | None -> 0 in
...
let test_e2e_stage3 () =
  let sample_rate = 44100 in
  let input = List.init 2000 (fun i -> Float.sin (2.0 *. Float.pi *. float_of_int i /. 100.0)) in
  ...
  let reconstructed_processed = Decoder.pass_through_silence reconstructed_segmented in
  
  (* Step 2': Mid-Side → LR (mono, so skip) *)
  let reconstructed_compressed, _ = Decoder.mid_side_to_lr reconstructed_processed reconstructed_processed is_stereo in
  
  (* Step 1': Decompress *)
  let final = Decoder.decompress_track reconstructed_compressed min_val max_val in
...
let test_encoder_mdct_level1 () =
  ...
  List.iter (fun seg ->
    check bool "Segment has raw_data with MDCT coefficients" 
      (match seg.Audiomodel.raw_data with Some bands -> List.length bands > 0 | None -> false) true;
    (* Check that MDCT coefficients length is reasonable (should be ~window_size/2) *)
    (match seg.Audiomodel.raw_data with
    | Some [mdct_coeffs] ->
      let window_size = match seg.Audiomodel.window_data with
      ...
      check bool "MDCT coefficients length is correct" 
        (abs (actual_len - expected_len) <= 1) true  (* Allow 1 sample difference *)
    | _ -> ())
  ) mdct_segments
...
let test_encoder_bands_split () =
  ...
  List.iter (fun seg ->
    check bool "Segment has multiple bands" (seg.Audiomodel.num_bands > 0) true;
    check bool "Segment has frequency_bands defined" 
      (List.length seg.Audiomodel.frequency_bands = seg.Audiomodel.num_bands) true;
...
let test_encoder_mdct_level2 () =
  ...
  List.iter (fun seg ->
    check bool "Segment has raw_data with second-level MDCT" 
      (match seg.Audiomodel.raw_data with Some bands -> List.length bands > 0 | None -> false) true;
    (* Number of bands should be preserved *)
    check bool "Number of bands preserved" 
      (seg.Audiomodel.num_bands > 0) true;
    (* Check that each band has MDCT coefficients *)
    (match seg.Audiomodel.raw_data with
    | Some bands ->
      List.iter (fun band ->
        check bool "Second-level MDCT band is non-empty" (List.length band > 0) true
      ) bands
    | None -> ())
  ) mdct2_segments
...
let test_encoder_quant_thresholds () =
  ...
  List.iter (fun seg ->
    check bool "Segment has quantization_levels" 
      (List.length seg.Audiomodel.quantization_levels = seg.Audiomodel.num_bands) true;
    check bool "Segment has band_ranges" 
      (List.length seg.Audiomodel.band_ranges = seg.Audiomodel.num_bands) true;
    (* Check that quantization levels are valid (>= 2) *)
    List.iter (fun n ->
      check bool "Quantization level is valid" (n >= 2) true
...
let test_e2e_stage4_real_wav () =
  ...
  let mono_samples = if wav_data.Wav.Parser.info.Wav.Parser.num_channels = 2 then
  ...
  let test_samples = if List.length mono_samples > 1000 then
  ...
  let compressed, _min_val, _max_val, _snr1 = Encoder.compress_track test_samples in
  ...
  check bool "Segments have quantization levels" 
    (List.for_all (fun seg -> List.length seg.Audiomodel.quantization_levels > 0) quant_segments) true;
  check bool "Segments have frequency bands" 
    (List.for_all (fun seg -> List.length seg.Audiomodel.frequency_bands > 0) quant_segments) true;
  ...
  let bands = Audiomodel.polybank_filter test_samples test_frequencies sample_rate in
  ...
  if List.length bands > 0 then
    check bool "Polybank first band is non-empty" (List.length (List.hd bands) > 0) true

(* Step 5: Decoder tests (optimized for speed) *)
(* Step 5.1: Decoder IMDCT Level 2 *)
let test_decoder_imdct_level2 () =
  let sample_rate = 44100 in
  let input = List.init 800 (fun i -> Float.sin (2.0 *. Float.pi *. float_of_int i /. 100.0)) in
  ...
  let band_segments = Encoder.split_frequency_bands ~frequency_boundaries_hz:test_frequencies sample_rate mdct_segments in
  let imdct_segments = Encoder.apply_imdct_to_bands band_segments in
  let mdct2_segments = Encoder.apply_mdct_level2 imdct_segments in
  (* Decoder: Apply IMDCT Level 2 *)
  let decoded_imdct2 = Decoder.apply_imdct_level2 mdct2_segments in
  check bool "Has decoded IMDCT Level 2 segments" (List.length decoded_imdct2 > 0) true;
  (* Check that segments have time-domain bands *)
  List.iter (fun seg ->
    check bool "Segment has raw_data" 
      (match seg.Audiomodel.raw_data with Some _ -> true | None -> false) true
  ) decoded_imdct2
...
let test_decoder_mdct_level1_to_bands () =
  ...
  check bool "Has decoded MDCT Level 1 segments" (List.length decoded_mdct1 > 0) true
...
let test_decoder_merge_bands () =
  ...
  List.iter (fun seg ->
    check bool "Segment has single band after merge" (seg.Audiomodel.num_bands = 1) true
  ) merged_segments
...
let test_decoder_imdct_final () =
  ...
  List.iter (fun seg ->
    check bool "Segment has window_data" 
      (match seg.Audiomodel.window_data with Some _ -> true | None -> false) true
  ) decoded_windows
...
let test_encoder_create_audio_file () =
  ...
  check int "AudioFile sample_rate matches" audio_file.Audiomodel.sample_rate sample_rate;
  check int "AudioFile bits_per_sample is 16" audio_file.Audiomodel.bits_per_sample 16;
  check bool "AudioFile has tracks" (List.length audio_file.Audiomodel.tracks > 0) true;
  (match audio_file.Audiomodel.tracks with
  | track :: _ ->
    check int "Track has segments" track.Audiomodel.num_segments (List.length quant_segments);
    check int "Track sample_rate matches" track.Audiomodel.sample_rate sample_rate;
...
let test_decoder_read_audio_file () =
  ...
  check int "Read segments count matches" (List.length segments) (List.length quant_segments);
  check bool "Read compression_params matches" 
    ...
    | Some (r_min, r_max), Some (e_min, e_max) -> 
      abs_float (r_min -. e_min) < 0.0001 && abs_float (r_max -. e_max) < 0.0001
    | None, None -> true
    | _ -> false) true
...
let test_e2e_audio_file_structure () =
  ...
  in
  (* Read AudioFile *)
  let (read_segments, read_sample_rate, read_compression_params, _read_original_length) = 
    Decoder.read_audio_file audio_file 
  in
  (* Verify all fields *)
  check int "E2E: sample_rate preserved" read_sample_rate sample_rate;
  check int "E2E: segments count preserved" (List.length read_segments) (List.length quant_segments);
  ...
  if List.length read_segments = List.length quant_segments then (
    List.iter2 (fun read_seg orig_seg ->
...
let test_deserialization () =
  ...
  check int "Deserialized num_tracks matches" 
    deserialized.Audiomodel.num_tracks audio_file.Audiomodel.num_tracks;
  ...
  check int "Deserialized bits_per_sample matches" 
    deserialized.Audiomodel.bits_per_sample audio_file.Audiomodel.bits_per_sample;
...
let test_roundtrip_serialization () =
  ...
  let mdct2_segments = Encoder.apply_mdct_level2 imdct_segments in
  let quant_segments = Encoder.select_quantization_thresholds mdct2_segments in
  let original_audio_file = Encoder.create_audio_file 
    ~compression_params:(Some (min_val, max_val))
    sample_rate 
    quant_segments 
  in
  ...
  (* Compare all fields *)
  check int "Roundtrip: num_tracks preserved" 
    deserialized.Audiomodel.num_tracks original_audio_file.Audiomodel.num_tracks;
  check int "Roundtrip: sample_rate preserved" 
    deserialized.Audiomodel.sample_rate original_audio_file.Audiomodel.sample_rate;
  check int "Roundtrip: bits_per_sample preserved" 
    deserialized.Audiomodel.bits_per_sample original_audio_file.Audiomodel.bits_per_sample;
  check int "Roundtrip: tracks count preserved" 
    (List.length deserialized.Audiomodel.tracks) (List.length original_audio_file.Audiomodel.tracks);
  (* Compare tracks *)
  (match deserialized.Audiomodel.tracks, original_audio_file.Audiomodel.tracks with
  | [des_track], [orig_track] ->
    ...
    check int "Roundtrip: track segments count preserved" 
      (List.length des_track.Audiomodel.segments) (List.length orig_track.Audiomodel.segments)
  | _ -> ())
...
let test_decoder_file () =
  ...
  check bool "Reconstructed signal is not empty" (List.length reconstructed > 0) true;
  (* Cleanup *)
  Sys.remove temp_file
...
let test_full_roundtrip () =
  ...
  let test_samples = if List.length samples > 2000 then
  ...
  (* Encode to file *)
  let _audio_file2 = Encoder.encode_to_file test_samples wav_sample_rate temp_file2 in
  (* Decode from file *)
  let (reconstructed_wav, _read_sample_rate2) = Decoder.decode_from_file temp_file2 in
  (* Check that reconstruction works *)
  check bool "Reconstructed WAV signal is not empty" (List.length reconstructed_wav > 0) true;
  (* Check length is reasonable *)
  let len_diff2 = abs (List.length reconstructed_wav - List.length test_samples) in
  check bool "Reconstructed WAV length is reasonable" (len_diff2 <= 2048) true;
  (* Check SNR for real WAV *)
  let min_len2 = min (List.length test_samples) (List.length reconstructed_wav) in
  let input_trimmed2 = List.init min_len2 (fun i -> List.nth test_samples i) in
  let reconstructed_trimmed2 = List.init min_len2 (fun i -> List.nth reconstructed_wav i) in
  ...
  (* SNR may be negative if noise > signal, but that's still a valid reconstruction *)
  check bool "Roundtrip with real WAV produces reconstruction" (not (Float.is_nan snr2)) true;
  (* Cleanup *)
...
let test_full_wav_roundtrip () =
  ...
  check bool "WAV has valid number of channels" (num_channels >= 1 && num_channels <= 2) true;
  
  (* Encode to custom format *)
  let temp_encoded = Filename.temp_file "test_full_wav_encode" ".audc" in
  let audio_file = Encoder.encode_to_file samples wav_sample_rate temp_encoded in
  
  (* Verify encoded file structure *)
  check int "Encoded file has 1 track" audio_file.Audiomodel.num_tracks 1;
  check int "Encoded sample_rate matches" audio_file.Audiomodel.sample_rate wav_sample_rate;
  ...
  (* Verify decoded data *)
  check int "Decoded sample_rate matches original" decoded_sample_rate wav_sample_rate;
  ...
  let max_expected_diff = (List.length samples / 10) + 2048 in  (* Allow up to 10% + 2048 samples difference *)
  ...
  check bool "SNR is valid" (not (Float.is_nan snr) && not (Float.is_infinite snr && snr < 0.0)) true;
  ...
  check int "Output WAV length matches" 
    (List.length output_wav_data.Wav.Parser.samples) (List.length reconstructed);
...
let () =
  run "Encoder/Decoder Tests" [
    "Step 1.1: Encoder Compression", [
      ...
      test_case "Compression roundtrip" `Quick test_encoder_compress_roundtrip;
    ...
    "Step 1.3: Encoder LR → Mid-Side", [
      test_case "Stereo conversion" `Quick test_encoder_lr_to_mid_side_stereo;
    ...
    "Step 2.1: Encoder Silence Replacement", [
      test_case "Silence replacement" `Quick test_encoder_silence_replacement;
      test_case "No replacement for loud signal" `Quick test_encoder_silence_no_replacement;
    ];
    ...
    "Step 3.1: Encoder Segmentation", [
      ...
      test_case "Short track padding" `Quick test_encoder_segmentation_short_track;
    ];
    "Step 3.2: Encoder Overlapping Windows", [
      test_case "Overlapping windows creation" `Quick test_encoder_overlapping_windows;
    ];
    ...
    "Step 3.4: E2E Test Stage 3", [
      test_case "Full cycle 1→2→3→4→4'→3'→2'→1' (without MDCT)" `Quick test_e2e_stage3;
    ];
    "Step 4.1: Encoder MDCT Level 1", [
      test_case "MDCT transform window to frequency domain" `Quick test_encoder_mdct_level1;
    ];
    "Step 4.2: Encoder Frequency Bands Split", [
      test_case "Split MDCT coefficients into frequency bands" `Quick test_encoder_bands_split;
    ];
    "Step 4.3: Encoder IMDCT Bands", [
      test_case "Apply IMDCT to frequency bands" `Quick test_encoder_bands_imdct;
    ];
    "Step 4.4: Encoder MDCT Level 2", [
      test_case "Apply second level MDCT to time-domain bands" `Quick test_encoder_mdct_level2;
    ];
    "Step 4.5: Encoder Quantization Thresholds", [
      test_case "Select quantization thresholds for bands" `Quick test_encoder_quant_thresholds;
    ];
    "Step 4.6: E2E Test Stage 4 with Real WAV", [
      test_case "Full MDCT pipeline with real WAV file" `Quick test_e2e_stage4_real_wav;
    ];
    "Step 5.1: Decoder IMDCT Level 2", [
      test_case "Apply IMDCT Level 2 to bands" `Quick test_decoder_imdct_level2;
    ];
    "Step 5.2: Decoder MDCT Level 1 to Bands", [
      test_case "Apply MDCT Level 1 to time-domain bands" `Quick test_decoder_mdct_level1_to_bands;
    ];
    "Step 5.3: Decoder Merge Frequency Bands", [
      test_case "Merge frequency bands back" `Quick test_decoder_merge_bands;
    ];
    "Step 5.4: Decoder Final IMDCT", [
      test_case "Apply final IMDCT to restore windows" `Quick test_decoder_imdct_final;
    ];
    "Step 6.1: Encoder Create AudioFile", [
      test_case "Create AudioFile structure from segments" `Quick test_encoder_create_audio_file;
    ];
    "Step 6.2: Decoder Read AudioFile", [
      test_case "Read AudioFile structure and extract segments" `Quick test_decoder_read_audio_file;
    ];
    "Step 6.3: E2E AudioFile Structure", [
      test_case "E2E: create → read → verify" `Quick test_e2e_audio_file_structure;
    ];
    "Step 7.1: Serialization", [
      test_case "Serialize AudioFile to bytes" `Quick test_serialization;
    ];
    "Step 7.2: Deserialization", [
      test_case "Deserialize AudioFile from bytes" `Quick test_deserialization;
    ];
    "Step 7.3: Roundtrip Serialization", [
      test_case "E2E: serialize → deserialize → compare" `Quick test_roundtrip_serialization;
    ];
    ...
    "Step 8.3: Full Roundtrip", [
      test_case "Full E2E: WAV → encode → file → decode → WAV" `Quick test_full_roundtrip;
    ];
    "Full WAV File Test", [
      test_case "Complete WAV file roundtrip with verification" `Quick test_full_wav_roundtrip;
    ];
  ]
