...
let test_serialization_quantized_data () =
  ...
  check int "Deserialized num_tracks matches"
    deserialized.Codec.Audiomodel.num_tracks
    audio_file.Codec.Audiomodel.num_tracks;
      ...
      | [ orig_track ] ->
          ...
          List.iter2
            (fun des_seg orig_seg ->
              ...
              | None ->
                  check bool "deserialized segment has quantized_data" false
                    true
              | Some des_bands -> (
                  match orig_seg.Codec.Audiomodel.quantized_data with
                  | None ->
                      check bool "original segment has quantized_data" false
                        true
                  | Some orig_bands ->
...
let () =
  run "Quantization Pipeline Tests"
    [
      ( "Quantization",
        [
          test_case "Encoder creates quantized_data" `Quick
            test_encoder_quantization_creates_data;
          test_case "Quantization levels are valid (> 0)" `Quick
            test_quantization_levels_valid;
        ] );
      ( "Serialization",
        [
          test_case "Quantized data serializes correctly" `Quick
            test_serialization_quantized_data;
        ] );
      ( "Dequantization",
        [
          test_case "Decoder dequantizes correctly" `Quick
            test_decoder_dequantization;
        ] );
      ( "Full Pipeline",
        [
          test_case "Full quantization roundtrip" `Quick
            test_full_quantization_roundtrip;
        ] );
    ]
