...
let test_segment_creation () =
  ...
  check int "Segment frequency_bands length"
    (List.length segment.AudioModel.frequency_bands)
    2;
  check int "Segment quantization_levels length"
    (List.length segment.AudioModel.quantization_levels)
    2
...
let test_segment_with_raw_data () =
  ...
  check bool "Segment has raw_data"
    (match segment.AudioModel.raw_data with Some _ -> true | None -> false)
    true;
  check bool "Segment has no quantized_data"
    (match segment.AudioModel.quantized_data with
    | Some _ -> false
    | None -> true)
    true
...
let test_empty_track () =
  ...
  check int "Empty track segments length"
    (List.length track.AudioModel.segments)
    0
...
let test_standard_frequencies () =
  ...
  check (float 0.1) "Last frequency is 20kHz"
    (List.nth freqs (List.length freqs - 1))
    20000.0
...
let test_polybank_filter_basic () =
  ...
  let signal =
    List.init 128 (fun i ->
        Float.sin (2.0 *. Float.pi *. float_of_int i /. 16.0))
  in
  let sample_rate = 44100 in
  let bands =
    AudioModel.polybank_filter signal AudioModel.standard_10band_frequencies_hz
      sample_rate
  in
  ...
  List.iter
    (fun band ->
      check int "Band has correct length" (List.length band)
        (List.length signal))
    bands
...
let test_polybank_filter_custom () =
  ...
  let signal =
    List.init 64 (fun i -> Float.sin (2.0 *. Float.pi *. float_of_int i /. 8.0))
  in
  let sample_rate = 44100 in
  ...
  check int "Custom polybank returns correct number of bands"
    (List.length bands) 3
...
let test_find_optimal_quantization_snr () =
  let signal =
    List.init 200 (fun i ->
        Float.sin (2.0 *. Float.pi *. float_of_int i /. 20.0))
  in
  let n, min_val, max_val =
    AudioModel.find_optimal_quantization ~snr_threshold:30.0 signal
  in
  (* Verify SNR is maintained *)
  let quantized, _, _ =
    Codec_utilities.Quantization.quantize_with_range signal n
  in
  let dequantized =
    Codec_utilities.Quantization.dequantize_with_range quantized n min_val
      max_val
  in
...
let test_find_optimal_quantization_high_threshold () =
  ...
  in
  let n, _, _ =
    AudioModel.find_optimal_quantization ~snr_threshold:60.0 signal
  in
...
let test_compress_track_basic () =
  ...
  check int "Compressed length matches" (List.length compressed)
    (List.length signal);
...
let test_decompress_track_basic () =
  ...
  check int "Decompressed length matches" (List.length decompressed)
  ...
  check (float 0.1) "Last value decompressed"
    (List.nth decompressed (List.length decompressed - 1))
    2.0
...
let test_compress_decompress_roundtrip () =
  ...
  check int "Roundtrip preserves length" (List.length decompressed)
    (List.length original);
  (* Values should be approximately recovered *)
  List.iter2
    (fun orig dec -> check float_approx "Roundtrip recovers original" orig dec)
...
let test_lr_mid_side_roundtrip () =
  ...
  let right =
    List.init 100 (fun i ->
        Float.cos (2.0 *. Float.pi *. float_of_int i /. 10.0))
  in
  let mid, side = AudioModel.lr_to_mid_side left right in
  let left_recovered, right_recovered = AudioModel.mid_side_to_lr mid side in
  check int "Roundtrip preserves left length"
    (List.length left_recovered)
    (List.length left);
  check int "Roundtrip preserves right length"
    (List.length right_recovered)
    (List.length right);
...
let test_lr_to_mid_side_mono () =
  ...
  List.iter2
    ...
    signal mid;
  List.iter (fun s -> check float_approx "Mono side is zero" s 0.0) side
    ...
    [
      ...
      ( "Polybank filter",
        [
          ...
          test_case "Polybank filter single band" `Quick
            test_polybank_filter_single_band;
        ] );
      ( "Window type FSM",
        [
          test_case "Determine window: Short" `Quick test_determine_window_short;
          test_case "Determine window: EndFocused" `Quick
            test_determine_window_end_focused;
          test_case "Determine window: StartFocused" `Quick
            test_determine_window_start_focused;
          test_case "Determine window: VeryLong" `Quick
            test_determine_window_very_long;
          test_case "Determine window: Long" `Quick test_determine_window_long;
          test_case "Determine window: edge cases" `Quick
            test_determine_window_edge_cases;
        ] );
      ( "Adaptive quantization",
        [
          test_case "Find optimal quantization: basic" `Quick
            test_find_optimal_quantization_basic;
          test_case "Find optimal quantization: SNR" `Quick
            test_find_optimal_quantization_snr;
          test_case "Find optimal quantization: constant" `Quick
            test_find_optimal_quantization_constant;
          test_case "Find optimal quantization: empty" `Quick
            test_find_optimal_quantization_empty;
          test_case "Find optimal quantization: high threshold" `Quick
            test_find_optimal_quantization_high_threshold;
        ] );
      ( "Track compression",
        [
          ...
          test_case "Compress-decompress roundtrip" `Quick
            test_compress_decompress_roundtrip;
          test_case "Compress track: constant" `Quick
            test_compress_track_constant;
