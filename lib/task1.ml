...
let solve1_modular min_divisors =
  let generate_triangles n =
    let rec gen acc i =
      ...
      else gen (triangle_number i :: acc) (i + 1)
    in
    gen [] 1
  in
  ...
  let limit = 20000 in
  generate_triangles limit
  |> filter_sufficient_divisors
  |> find_first
...
let solve1_map min_divisors =
  ...
  let with_divisors = List.map (fun tri -> (tri, count_divisors tri)) triangles in
  let filtered = List.filter (fun (_, divs) -> divs > min_divisors) with_divisors in
  match filtered with
  | [] -> failwith "No triangle number found"
  | (tri, _) :: _ -> tri
...
let solve1_for_loop min_divisors =
  ...
  match !result with
  | Some x -> x
  | None -> failwith "No triangle number found"
...
let solve1_seq min_divisors =
  ...
  let with_divisors = Seq.map (fun tri -> (tri, count_divisors tri)) triangles in
  let filtered = Seq.filter (fun (_, divs) -> divs > min_divisors) with_divisors in
  match filtered () with
