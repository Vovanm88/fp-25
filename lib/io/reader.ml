...
let list_to_array bytes = (Array.of_list bytes, 0)
...
let read_int32_array (arr, idx) =
  if idx + 4 > Array.length arr then failwith "Not enough bytes to read int32"
  else
    let b0 = arr.(idx) in
    let b1 = arr.(idx + 1) in
    let b2 = arr.(idx + 2) in
    ...
    let value =
      if value land 0x80000000 <> 0 then value - 0x100000000 else value
    in
    (value, (arr, idx + 4))

(* Read int16 from array - O(1) *)
let read_int16_array (arr, idx) =
  if idx + 2 > Array.length arr then failwith "Not enough bytes to read int16"
  else
    let b0 = arr.(idx) in
    let b1 = arr.(idx + 1) in
...
let read_uint8_array (arr, idx) =
  if idx >= Array.length arr then failwith "Not enough bytes to read uint8"
  else (arr.(idx), (arr, idx + 1))

(* Read float from array - O(1) *)
let read_float_array (arr, idx) =
  if idx + 4 > Array.length arr then failwith "Not enough bytes to read float"
  else
...
let read_from_file filename = read_bytes_from_file filename
...
let read_chunk bytes =
  ...
  if idx + size > Array.length arr then
    failwith
      ("Not enough bytes to read chunk data: need " ^ string_of_int size
     ^ ", got "
      ^ string_of_int (Array.length arr - idx))
  else
...
let find_chunk bytes chunk_id =
  ...
  let rec search idx =
    if idx + 8 > Array.length arr then
      (* Need at least 4 bytes ID + 4 bytes size *)
      None
    else
      let id, size, _ = read_chunk_header (array_to_list (arr, idx)) in
      if id = chunk_id then
        (* Found it, read the data *)
        if idx + 8 + size > Array.length arr then None
        else
        ...
        idx + 8 + size > Array.length arr
      then None
      else search (idx + 8 + size)
  in
  search start_idx
