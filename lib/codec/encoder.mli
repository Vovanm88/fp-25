...
val compress_stereo_tracks : float list -> float list -> (float list * float list) * (float * float) * (float * float) * float
...
val replace_silence_after_loud : ?window_size:int -> ?loud_threshold:float -> ?quiet_threshold:float -> ?quiet_duration_ms:float -> int -> float list -> float list * bool list
...
val split_frequency_bands : ?frequency_boundaries_hz:float list -> int -> segment list -> segment list
...
val select_quantization_thresholds : ?snr_threshold_db:float -> segment list -> segment list
...
val encode_to_file : ?snr_threshold_db:float -> float list -> int -> string -> Audiomodel.audio_file
