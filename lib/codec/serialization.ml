...
let serialize_segment (seg : Audiomodel.segment) =
  ...
  let parts = Writer.write_uint8 0 :: parts in
  (* Always write 0 - we don't save window_data *)
  (* start_sample: optional int *)
  let parts =
    (match seg.Audiomodel.start_sample with
  ...
  let parts =
    (match seg.Audiomodel.attack_ms with
    | None -> Writer.write_uint8 0
  ...
  let parts =
    (match seg.Audiomodel.release_ms with
    | None -> Writer.write_uint8 0
    | Some v -> Writer.write_uint8 1 @ Writer.write_float v)
    :: parts
  in
  ...
  let parts =
    (match seg.Audiomodel.original_length with
    | None -> Writer.write_uint8 0
    | Some v -> Writer.write_uint8 1 @ Writer.write_int32 v)
    :: parts
  in
  ...
  let parts =
    Writer.write_int32 (List.length seg.Audiomodel.frequency_bands) :: parts
  in
  ...
  let parts =
    Writer.write_int32 (List.length seg.Audiomodel.band_ranges) :: parts
  in
  (* Build in reverse, then concat *)
  ...
  let parts =
    Writer.write_list_int32 seg.Audiomodel.quantization_levels :: parts
  in
        ...
        let quant_bytes =
          let rec fold_bands i acc =
            if i >= Array.length bands_arr then List.rev acc
              ...
              let packed =
                Quantization.encode_quantized_indices band quant_level
              in
              let band_bytes =
                Writer.write_int32 (List.length band)
                @ Writer.write_list_uint8 packed
              in
  ...
  let parts =
    ...
    | Some bands ->
        ...
        let raw_bytes_rev =
          Array.fold_left
            (fun acc band -> Writer.write_list_float band :: acc)
            [] bands_arr
        in
...
let deserialize_segment_array (arr, idx) =
  ...
  let _flag =
    let v, s = Reader.read_uint8_array !state in
    state := s;
    v
  in
  ...
  let start_sample =
    ...
    if flag = 0 then None
      ...
      Some
        (let v, s = Reader.read_int32_array !state in
         state := s;
         v)
  in
  ...
  let attack_ms =
    ...
    if flag = 0 then None
      ...
      Some
        (let v, s = Reader.read_float_array !state in
         state := s;
         v)
  in
  ...
  let quantization_levels =
    let v, s = Reader.read_list_int32_array !state in
    state := s;
    v
  in
      ...
      let rec read_bands n acc st =
        if n = 0 then (Some (List.rev acc), st)
          ...
          if band_index >= Array.length levels_arr then
            failwith
              (Printf.sprintf "Band index %d >= quantization_levels length %d"
            ...
            if quant_level <= 0 then
              failwith
                (Printf.sprintf "Invalid quantization level %d at index %d"
                   quant_level band_index)
            else
              (* Decode using bit unpacking *)
              let band =
                Quantization.decode_quantized_indices packed_bytes num_elements
                  quant_level
              in
  ...
  let segment =
    {
      Audiomodel.window_type;
      Audiomodel.window_data;
      Audiomodel.start_sample;
      Audiomodel.end_sample;
      Audiomodel.attack_ms;
      Audiomodel.release_ms;
      Audiomodel.original_length;
      Audiomodel.num_bands;
      Audiomodel.frequency_bands;
      Audiomodel.band_ranges;
      Audiomodel.quantization_levels;
      Audiomodel.quantized_data;
      Audiomodel.raw_data;
    }
  in
  (segment, !state)
...
let serialize_track track =
  ...
  (* segments *)
  let parts =
    Writer.write_int32 (List.length track.Audiomodel.segments) :: parts
  in
  let parts =
    List.concat (List.map serialize_segment track.Audiomodel.segments) :: parts
  in
  (* length_samples *)
  ...
  let parts =
    ...
    | Some (min_val, max_val) ->
        Writer.write_uint8 1 @ Writer.write_float min_val
        @ Writer.write_float max_val)
    :: parts
  in
  (* Simple approach: reverse parts and concatenate *)
  (* parts is in reverse order (newest first), so List.rev gives correct order *)
...
let deserialize_track_array (arr, idx) =
  ...
  let track =
    {
      Audiomodel.num_segments;
      Audiomodel.segments;
      Audiomodel.length_samples;
      Audiomodel.sample_rate;
      Audiomodel.compression_params;
    }
  in
  (track, !state)
...
let serialize_audio_file audio_file =
  ...
  (* tracks *)
  let parts =
    Writer.write_int32 (List.length audio_file.Audiomodel.tracks) :: parts
  in
  let parts =
    List.concat (List.map serialize_track audio_file.Audiomodel.tracks) :: parts
  in
  (* bits_per_sample *)
  let parts =
    Writer.write_int32 audio_file.Audiomodel.bits_per_sample :: parts
  in
  (* sample_rate *)
...
let deserialize_audio_file bytes =
  ...
  if magic <> "AUDC" then failwith ("Invalid magic: expected AUDC, got " ^ magic);
  if version <> 1 then failwith ("Unsupported version: " ^ string_of_int version);
  state := Reader.list_to_array rem;
  ...
  let audio_file =
    {
      Audiomodel.num_tracks;
      Audiomodel.tracks;
      Audiomodel.bits_per_sample;
      Audiomodel.sample_rate;
    }
  in
