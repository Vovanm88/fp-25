...
let test_fft_shift () =
  ...
  check (list float_approx) "FFT shift preserves length" (List.length result)
    (List.length input);
...
let test_single_element () =
  let single = [ 5.0 ] in
  check (list float_approx) "DFT on single element" (Dsp.dft_transform single)
    [ 5.0 ];
  check (list float_approx) "FFT on single element" (Dsp.fft_transform single)
    [ 5.0 ];
