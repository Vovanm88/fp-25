...
let print_usage () =
  ...
  Printf.printf
    "  --input, -i FILE      Input file (WAV for encode, MLAC for decode)\n";
  Printf.printf
    "  --output, -o FILE     Output file (MLAC for encode, WAV for decode)\n";
  Printf.printf
    "  --quality, -q FLOAT   Quality threshold (SNR in dB, default: 40.0)\n";
...
let parse_args () =
  ...
  let set_mode m =
    if !mode <> None then
      raise
        (Invalid_args
           "Mode can only be specified once (use either --encode or --decode)")
    else mode := Some m
  in
  ...
  try
    ...
    let mode_val =
      match !mode with
      | None ->
          raise (Invalid_args "Mode is required (use --encode or --decode)")
      | Some m -> m
    in
    ...
    let input_val =
      ...
      | None ->
          raise (Invalid_args "Input file is required (use --input or -i)")
      | Some f -> f
    in
    ...
    let output_val =
      ...
      | None ->
          raise (Invalid_args "Output file is required (use --output or -o)")
      | Some f -> f
    in
    ...
    | Encode ->
        ...
        if not (ends_with output_val ".mlac") then
          raise
            (Invalid_args "Output file must be a .mlac file for encode mode")
    | Decode ->
